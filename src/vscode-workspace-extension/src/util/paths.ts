...
export function getDotNetStorePath(): string {
  return path.join(home, ".dotnet", "corefx", "cryptography", "x509stores", "my");
}
...
export function getOpenSslTrustDir(): string {
  return (
    ...
    path.join(home, ".aspnet", "dev-certs", "trust")
  );
}

/**
 * PFX filename in the .NET store — matches what .NET's OpenSslDirectoryBasedStoreProvider expects.
 */
...
/**
 * PEM filename in the OpenSSL trust directory — matches UnixCertificateManager naming.
 */
