...
Read-only diagnostics: which backends are available, what `--backend auto` would pick, host platform-store state, and (on Linux) `openssl` / `certutil` presence on PATH.
